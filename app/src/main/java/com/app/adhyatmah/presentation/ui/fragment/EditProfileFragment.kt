...
class EditProfileFragment : BaseFragment<FragmentEditProfileBinding>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        oldProfileData = Preferences.getCustomModelPreference(requireContext(), IS_PROFILE_DATA) ?: return
    ...
    private fun updateProfile(image: String) {
        val request = EditProfileRequest(
            ...
            image = image.ifEmpty { Preferences.getStringPreference(requireContext(), PROFILE_IMG).orEmpty() }
        )
        ...
        return with(binding) {
            when {
                nameSectionEt.getString().isBlank() -> {
                    nameSectionEt.error = "First name is required"; nameSectionEt.requestFocus(); false
                }
                etLastName.getString().isBlank() -> {
                    etLastName.error = "Last name is required"; etLastName.requestFocus(); false
                }
                phoneNumber.getString().isBlank() -> {
                    phoneNumber.error = "Phone number is required"; phoneNumber.requestFocus(); false
                }
                phoneNumber.getLength() < 7 -> {
                    phoneNumber.error = "Enter a valid phone number"; phoneNumber.requestFocus(); false
                }
                else -> true
            }
        }
    ...
    override fun onRequestPermissionsResult(
        ...
        /*        if (requestCode == CAMERA_REQUEST_CODE) {
                    ...
                    Toast.makeText(requireContext(), "Camera nhi hai ", Toast.LENGTH_SHORT).show()
                }*/
    }
    ...
    private fun setObserver() {
            ...
            when (it.status) {
                Status.SUCCESS -> {
                    if (it.data?.code == 200) {
                        ...
                        Toast.makeText(requireContext(), it.data?.message ?: "Update failed", Toast.LENGTH_SHORT).show()
                    }
        ...
        viewmodel.getProfileData().observe(viewLifecycleOwner) {
            when (it.status) {
                Status.SUCCESS -> {
                    if (it.data?.code == 200) {
                        Preferences.setCustomModelPreference(requireContext(), IS_PROFILE_DATA, it.data.payload)
                        findNavController().navigateUp()
                    }
                    ProcessDialog.dismissDialog(true)
                }
        ...
        viewmodel.uploadProfileImgData().observe(viewLifecycleOwner) {
            when (it.status) {
                Status.SUCCESS -> {
                    if (it.data?.code == 200) {
//                        viewmodel.hitAPIProfileImageData(customerID)
                        Preferences.setStringPreference(requireContext(), PROFILE_IMG, it.data.payload?.url)
                        /*if (isTextUpdated) */updateProfile(it.data.payload?.url ?: "") /*else findNavController().navigateUp()*/
                ...
                Status.ERROR -> {
                    Snackbar.make(requireView(), it.message ?: "Upload error", Snackbar.LENGTH_SHORT).show()
                    ProcessDialog.dismissDialog(true)
                }
            }
        }
        ...
        viewmodel.getProfileImgData().observe(viewLifecycleOwner) {
            when (it.status) {
                ...
                Status.ERROR -> {
                    Snackbar.make(requireView(), it.message ?: "Fetch error", Snackbar.LENGTH_SHORT).show()
                    ProcessDialog.dismissDialog(true)
                }
            }
        }
