...
class MangeAddressFragment : BaseFragment<FragmentMangeAddressBinding>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        Log.d("TAG", "initView from: $from")
          fetchAddressList()
        Log.d("TAG", "inhhitView: $token   id is there  $id")
        setObserver()
            ...
            if (type == "mainLayoutClick") {
                if (from=="bookPanditji"||from=="home"){
                    val bundle = Bundle().apply {
                        putParcelable("resultKey", data)
                    }
                    setFragmentResult("requestKey", bundle)
                    parentFragmentManager.popBackStack()
                }else{
//                    if (!cardId.isNullOrEmpty() && !id.isNullOrEmpty()) {
                        val bundle = Bundle().apply {
                            putString(CART_ID, cardId)
                            putString(ADDRESS_ID, id)
                            putString("address",address_data.name+" "+address_data.address1+" "+address_data.address2)

                        }
                        Log.d("TAG", "setAdapter:$id \n card id $cardId ")
                        findNavController().navigate(R.id.action_mangeAddressFragment_to_paymentMethodFragment, bundle)

//                    }
//                    else {
//                        Toast.makeText(requireContext(), "Cart ID or Address ID is missing", Toast.LENGTH_SHORT).show()
//                    }
                }
            } else {
   ...
   fun deleteAddress(id :String){
       var token = Preferences.getStringPreference(requireContext(), ACCESS_TOKEN)
        var request = DeleteAddressRequest()
...
/*
        setObserver()
        val token = ManageAddressRequest("b3cf17424bb2a94016acd98fc22e6f1d")
        profileViewModel.getCustomerAddressData(token)
*/
        ...
        binding.addBtn.setOnClickListener {
            ...
            if (token.isNullOrEmpty()) {
                showLoginPrompt()
                // Toast.makeText(requireContext(), "Token is required", Toast.LENGTH_SHORT).show()
            }else{
                val bundle = Bundle()
                    ...
                    when (statusCode) {
                        200 -> {
                            ...
                            if(data.isNullOrEmpty()){
                               ...
                               // manageAddListAdapter.updateItems(data)
