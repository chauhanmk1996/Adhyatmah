...
/*
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        if (actionType == "delete") {
            ...
            binding.btnConfirm.setOnClickListener {
                viewModel.hitDeleteApi(deleteToken) // You must define this
            }
        } else {
            ...
            when (it.status) {
                Status.SUCCESS -> {
                    ...
                    if (it.data?.status==1){
                        if (it.data.code == 200){
                            dismiss()
                            Preferences.removeAllPreferencesExcept(requireContext(), listOf(FCM_TOKEN, SELECTED_LANGUAGE_NAME, SELECTED_LANGUAGE_CODE))
                            val intent = Intent(requireActivity(), LoginActivity::class.java)
                            intent.putExtra("screen", "login") // <--- Tell LoginActivity to show login
                            intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        actionType = arguments?.getString("type") // ✅ You missed this
    }
